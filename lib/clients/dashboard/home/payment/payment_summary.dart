...
import 'package:pragyan_cdc/api/therapist_api.dart';
...
class _PaymentSummaryState extends State<PaymentSummary> {
  Future<void> _bookAppointment() async {
    try {
      ...
      await TherapistApi().bookAppointmentApi(widget.bookingData);
  ...
  Widget build(BuildContext context) {
    ...
    widget.bookingData["prag_booking"].forEach((booking) {
      booking["prag_bookingdatetime"].forEach((date, slots) {
        ...
        slots.forEach((slot) {
          double cost = double.parse(slot[1]);
          totalCost += cost;
          totalSessions++;
                      ...
                      ...costBreakdown.entries.map((entry) {
                        return Column(
                          children: [
                            ...
                            ), // Add a divider between each row for better separation
                          ],
                        );
                      }).toList(),
                    ],
